...
class Extract:
    def __init__(self, api_url, params, headers, bucket_name):
        ...
        self.gcs_handler = GoogleCloudStorage(bucket_name)
    ...
    def save_to_gcs(self, dataframe, file_name):
        self.gcs_handler.save_dataframe_to_csv(file_name, dataframe)

    def execute(self):
        data = self.make_api_request()
        df, csv_name = self.save_to_df(data)
        self.save_to_gcs(df, f"raw/{csv_name}")
        print(f'Exported to GCS: {csv_name}')
        return csv_name